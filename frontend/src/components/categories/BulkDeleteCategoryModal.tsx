/**
 * Modal de confirmación para eliminar múltiples categorías.
 */
import { useState } from 'react'
import { AlertTriangle, Trash2, X } from 'lucide-react'
import { Modal, Button } from '../ui'

interface BulkDeleteCategoryModalProps {
  isOpen: boolean
  onClose: () => void
...
export default function BulkDeleteCategoryModal({
  isOpen,
  onClose,
  onConfirm,
  count,
}: BulkDeleteCategoryModalProps) {
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="mb-1 text-sm font-medium text-red-900 dark:text-red-200">
                Se eliminarán {count} categoría{count === 1 ? '' : 's'}.
              </p>
              <p className="text-xs text-red-700 dark:text-red-400">
                Si alguna categoría tiene subcategorías asociadas, también se eliminarán.
                Seleccionar una categoría padre no selecciona visualmente a sus hijas, pero el
                backend replica el borrado individual en cascada.
              </p>
            </div>
          </div>
        </div>
