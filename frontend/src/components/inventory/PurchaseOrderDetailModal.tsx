        ...
        <div className="flex-1 overflow-y-auto px-4 lg:px-6 py-4 lg:py-5 min-h-0">
          {isLoading ? (
            ...
            <div className="space-y-5">
              ...
              {/* Cards mobile para evitar tabla cortada */}
                ...
                {order.items.map((item) => (
                  <article key={item.id} className="rounded-xl border border-gray-200 bg-white p-3 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 truncate text-sm font-medium text-gray-900 dark:text-gray-100" title={item.product_description ?? '—'}>
                        {item.product_description ?? '—'}
                      </p>
                      {item.category_name && (
                        <p className="text-xs text-gray-500 dark:text-gray-400">{item.category_name}</p>
                      )}
                    </div>

                    <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                      <DataPill label="Stock sist." value={String(item.system_stock)} />
                      <DataPill label="Contado" value={item.counted_stock != null ? String(item.counted_stock) : '—'} />
                      <DataPill label="Cant. pedir" value={String(item.quantity_to_order)} valueClass="text-primary-700 dark:text-primary-300 font-semibold" />
                      <DataPill label="IVA %" value={`${item.iva_rate}%`} />
                    </div>

                    <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                      <DataPill label="Precio costo" value={formatCurrency(item.unit_cost)} mono />
                      <DataPill label="Subtotal" value={formatCurrency(item.subtotal)} mono valueClass="text-primary-700 dark:text-primary-300 font-semibold" />
                    </div>
                  </article>
                ))}
              </div>
          ...
          <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-2 lg:gap-3 px-4 lg:px-6 py-3 lg:py-4 border-t border-gray-200 dark:border-gray-700 flex-shrink-0">
            ...
            <div className="grid grid-cols-2 lg:flex lg:flex-wrap items-stretch lg:items-center gap-2 lg:gap-3">
              <button
                ...
                className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 text-xs lg:text-sm font-medium border border-violet-300 dark:border-violet-600 rounded-lg text-violet-700 dark:text-violet-300 bg-violet-50 dark:bg-violet-900/20 hover:bg-violet-100 dark:hover:bg-violet-900/35 transition-colors"
              >
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 text-xs lg:text-sm font-medium border border-sky-300 dark:border-sky-600 rounded-lg text-sky-700 dark:text-sky-300 bg-sky-50 dark:bg-sky-900/20 hover:bg-sky-100 dark:hover:bg-sky-900/35 transition-colors"
              >
              ...
              {order.status === 'draft' && onEdit && (
                <button
                  ...
                  className="col-span-2 lg:col-span-1 inline-flex items-center justify-center gap-1.5 px-3 py-1.5 text-xs lg:text-sm font-medium border border-amber-300 dark:border-amber-600 text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/20 rounded-lg hover:bg-amber-100 dark:hover:bg-amber-900/35 transition-colors"
                >
              ...
              {order.status === 'draft' && (
                <button
                  ...
                  className="col-span-2 lg:col-span-1 inline-flex items-center justify-center gap-1.5 px-3.5 lg:px-4 py-1.5 text-xs lg:text-sm font-semibold border border-emerald-600 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 hover:border-emerald-700 transition-colors"
                >
