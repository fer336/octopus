/**
 * Modal de confirmación para eliminar múltiples clientes.
 */
import { useState } from 'react'
import { AlertTriangle, Trash2, X } from 'lucide-react'
import { Modal, Button } from '../ui'

interface BulkDeleteClientModalProps {
  isOpen: boolean
  onClose: () => void
...
export default function BulkDeleteClientModal({
  isOpen,
  onClose,
  onConfirm,
  count,
}: BulkDeleteClientModalProps) {
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="mb-1 text-sm font-medium text-red-900 dark:text-red-200">
                Se eliminarán {count} cliente{count === 1 ? '' : 's'}.
              </p>
              <p className="text-xs text-red-700 dark:text-red-400">
                El borrado actual es soft delete: el cliente deja de aparecer en listados,
                pero sus saldos y datos históricos no se modifican.
              </p>
            </div>
          </div>
        </div>
