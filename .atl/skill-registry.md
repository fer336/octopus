...
| Skill | Source | Triggers |
|-------|--------|----------|
| **gsap-core** | `~/.agents/skills/gsap-core` | JavaScript animation, GSAP tweens, gsap.to()/from()/fromTo(), easing, stagger |
| **gsap-react** | `~/.agents/skills/gsap-react` | GSAP + React/Next.js, useGSAP hook, refs, cleanup |
| **gsap-timeline** | `~/.agents/skills/gsap-timeline` | Sequencing animations, gsap.timeline(), position parameter, choreography |
| **gsap-scrolltrigger** | `~/.agents/skills/gsap-scrolltrigger` | Scroll-linked animations, pinning, scrub, parallax |
| **gsap-plugins** | `~/.agents/skills/gsap-plugins` | GSAP plugin registration, ScrollTrigger, Flip, Draggable, SplitText, ScrollSmoother |
| **gsap-frameworks** | `~/.agents/skills/gsap-frameworks` | GSAP + Vue/Svelte, lifecycle, onMounted, cleanup |
| **gsap-performance** | `~/.agents/skills/gsap-performance` | Optimizing GSAP animations, preventing jank, smooth 60fps |
| **gsap-utils** | `~/.agents/skills/gsap-utils` | gsap.utils.clamp, mapRange, random, snap, wrap, pipe |

### 🤖 AI & Agents (LangChain/LangGraph/Deep Agents)

| Skill | Source | Triggers |
|-------|--------|----------|
| **langgraph-fundamentals** | `~/.agents/skills/langgraph-fundamentals` | Writing LangGraph code, StateGraph, state schemas, nodes, edges, streaming |
| **langchain-fundamentals** | `~/.agents/skills/langchain-fundamentals` | Creating LangChain agents, define tools, middleware |
| **langchain-dependencies** | `~/.agents/skills/langchain-dependencies` | Setting up projects, package versions, installation for LangChain/LangGraph/LangSmith |
| **langchain-middleware** | `~/.agents/skills/langchain-middleware` | Human-in-the-loop, custom middleware, structured output |
| **langchain-rag** | `~/.agents/skills/langchain-rag` | Building RAG systems, document loaders, embeddings, vector stores |
| **langgraph-human-in-the-loop** | `~/.agents/skills/langgraph-human-in-the-loop` | interrupt(), Command(resume=), approval workflows, error handling |
| **langgraph-persistence** | `~/.agents/skills/langgraph-persistence` | State persistence, checkpointers, thread_id, time travel |
| **deep-agents-core** | `~/.agents/skills/deep-agents-core` | Building Deep Agents applications, create_deep_agent(), harness architecture |
| **deep-agents-memory** | `~/.agents/skills/deep-agents-memory` | Deep Agents memory/persistence, StateBackend, StoreBackend |
| **deep-agents-orchestration** | `~/.agents/skills/deep-agents-orchestration` | Subagents, task planning, human approval, HITL |
| **framework-selection** | `~/.agents/skills/framework-selection` | Choosing LangChain vs LangGraph vs Deep Agents framework layer |

### 📈 Marketing, SEO & Growth

| Skill | Source | Triggers |
|-------|--------|----------|
| **seo-audit** | `~/.agents/skills/seo-audit` | SEO audit, technical SEO, on-page SEO, meta tags, ranking issues |
| **programmatic-seo** | `~/.agents/skills/programmatic-seo` | SEO-driven pages at scale, template pages, location pages |
...
- React/TS: `camelCase` for variables/functions, `PascalCase` for components
- Database: `snake_case` for tables and columns
- Code: English identifiers, Spanish comments/UI/docs
- Git: Spanish conventional commits (`feat:`, `fix:`, `refactor:`, `docs:`, `style:`, `test:`)

### Architecture Patterns
- Clean/Hexagonal-inspired: `routers/` → `services/` → `models/`
- Async everywhere (FastAPI async, SQLAlchemy async)
- UUID primary keys with soft delete (`deleted_at`)
